...
def LP(M,J,w,p):
    ...
    # for i in M:
    #     for s in range(T):
    #         constr_expr = gp.quicksum(x[i, j, s] for j in J)
    #         model.addConstr(constr_expr <= 1)
    ...
    # max(0, t - p[j][i] + 1), min(T, t + p[j][i]) + 1

    # for i in M:
    #     # random_number = random.randint(1, T)
    #     t = T
    #     # for t in range(random_number):
    #     constr_expr = 0
    #     for j in J:
    #         for s in range(max(0, t - p[j][i] + 1), t):
    #             constr_expr += x[i, j, s]
